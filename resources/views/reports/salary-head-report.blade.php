...
<div class="single-pro-review-area mt-t-30 mg-b-15">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{route('report.salaryHeadReport')}}" method="GET" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="salary_head">Select Salary Head</label>
                            <select name="salary_head" id="salary_head" class="form-control" required>
                                <option value="">Select Salary Head</option>
                                @foreach($SalaryHead as $head)
                                    <option value="{{ $head->id }}" {{ request('salary_head') == $head->id ? 'selected' : '' }}>{{ $head->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="appointment_type" id="appointment_type" class="form-control">
                                ...
                                @foreach($appointmenttypes as $type)
                                    <option value="{{ $type->name }}" {{ request('name') == $type->name ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="pf_category" id="pf_category" type="text" class="form-control" placeholder="" value="">
