...
<div class="card">
    <div class="card-header">
        ...
        <a class="btn btn-sm btn-outline-primary float-right mr-1" href="{{route("loan.processed_loan_list")}}">
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <select name="installment_month" id="installment_month" class="form-control" required>
                            <option value="">Select Month</option>
                            <option value="1" {{ old('installment_month') == '1' ? 'selected' : '' }}>January</option>
                            <option value="2" {{ old('installment_month') == '2' ? 'selected' : '' }}>February</option>
                            <option value="3" {{ old('installment_month') == '3' ? 'selected' : '' }}>March</option>
                            <option value="4" {{ old('installment_month') == '4' ? 'selected' : '' }}>April</option>
                            <option value="5" {{ old('installment_month') == '5' ? 'selected' : '' }}>May</option>
                            <option value="6" {{ old('installment_month') == '6' ? 'selected' : '' }}>June</option>
                            <option value="7" {{ old('installment_month') == '7' ? 'selected' : '' }}>July</option>
                            <option value="8" {{ old('installment_month') == '8' ? 'selected' : '' }}>August</option>
                            <option value="9" {{ old('installment_month') == '9' ? 'selected' : '' }}>September</option>
                            <option value="10" {{ old('installment_month') == '10' ? 'selected' : '' }}>October</option>
                            <option value="11" {{ old('installment_month') == '11' ? 'selected' : '' }}>November</option>
                            <option value="12" {{ old('installment_month') == '12' ? 'selected' : '' }}>December</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <select name="installment_year" id="installment_year" class="form-control" required>
                            <option value="">Select Year</option>
                            @for ($year = 2024; $year <= 2035; $year++)
                                <option value="{{ $year }}" {{ old('installment_year') == $year ? 'selected' : '' }}>{{ $year }}</option>
                            @endfor
                        </select>
            ...
            <tr>
                <td>
                    @if($advance->advancesprocessdata)
                        ...
                        @endif
                    @else
                        <input name="datas[{{$key}}][advance_id]" type="checkbox" value="{{$advance->id}}" onclick="chkBox(this)" id="checkbox-{{$advance->id}}"/>
                        <input name="allrows[{{$key}}][advance_id]" type="checkbox" value="{{$advance->id}}" id="checkbox-{{$advance->id}}" checked hidden />
                    @endif
                </td>
                <td>{{ $key+1 }}</td>
                <td>{{ optional($advance->employee)->first_name ?? 'N/A' }} ({{ optional($advance->employee)->code ?? 'N/A' }})</td>
                <td>{{ optional($advance->advanceType)->type_name ?? 'N/A' }}</td>
                <td>
                ...
                <td class="text-right">
                    <input type="number" name="datas[{{$key}}][monthly_premium]" value={{$advance->interest_emi }} disabled="disabled">
                </td>
    ...
    {{$advances->links()}}
    <input type="submit" class="btn btn-primary btn-sm" value="Submit for Processing" id="process">
    <a class="btn btn-outline-primary btn-sm" href="{{route('advance.processed_data_list')}}">
